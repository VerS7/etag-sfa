<template>
  <v-card v-if="item">
    <v-card-title class="font-weight-bold">Создание</v-card-title>
    <v-divider></v-divider>
    <v-form ref="itemForm">
      <v-card-text>
        ...
        <v-row dense class="mb-2">
          <v-text-field
            ...
            label="Название"
            :rules="[(v) => !!v || 'Отсутствует название товара.']"
            required
          ></v-text-field>
        </v-row>
        <v-row dense>
          <v-col cols="12" md="6" class="mb-2"
            ><v-text-field
              ...
              label="Штрих-код"
              :rules="[(v) => !!v || 'Отсутствует штрих-код.']"
              type="number"
              hide-spin-buttons
              required
            ></v-text-field>
...
import type { ProductUpdate } from '@/apiFetch'
...
let newItem = ref<ProductUpdate>({ ...props.item })

const emits = defineEmits<{
  (e: 'submit', data: ProductUpdate): void
  (e: 'discard'): void
}>()
...
async function validateForm() {
  ...
  emits('submit', formatProduct(newItem.value))
}
</script>
