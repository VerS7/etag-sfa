...
router = APIRouter(
    prefix="/token", tags=["Token Auth"], dependencies=[Depends(auth_credentials)]
)
...
async def get_tokens(session: AsyncSession = Depends(get_session)):
    ...
    if len(tokens) == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Tokens not found!"
        )
    return tokens


@router.get("/{name}", response_model=AuthToken)
async def get_token(name: str, session: AsyncSession = Depends(get_session)):
