...
class DefaultImage(BaseImageTemplate):
    def __init__(self):
        ...
        self.bg_image = os.path.join(
            os.path.dirname(__file__), "assets/default/default.png"
        )
        self.font = os.path.join(os.path.dirname(__file__), "assets/default/tahoma.ttf")
        self.bold_font = os.path.join(
            os.path.dirname(__file__), "assets/default/tahoma_bold.ttf"
        )
    ...
    async def generate_image(
        self, data: Product, size: str | None = None, scale: float | None = None
    ) -> bytes:
        ...
        # Единица измерения
        draw.text(
            text=f"{data.unit if '.' in data.unit else f'{data.unit}.'}",
            font=self.info_font,
            xy=(90, 335),
            fill=self.text_color,
            anchor="mm",
        ...
        if scale is not None:
            bg_image = bg_image.resize(
                (int(bg_image.size[0] * scale), int(bg_image.size[1] * scale))
            )
